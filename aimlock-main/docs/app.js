...
function applyFilters(resetPage = false) {
  ...
  els.countLabel.innerHTML =
    state.favoritesOnly && loggedIn
      ? `즐겨찾기 <strong>${total}</strong>개 · ${state.page}/${totalPages} 페이지`
      : `총 <strong>${total}</strong>개 · ${state.page}/${totalPages} 페이지`;
...
function initUrlParams() {
  ...
  if (sort !== "popular" && sort !== "latest") return;
